...
@section('subtitle', 'Informasi lengkap jadwal sesi')
    ...
    <div class="bg-white rounded-xl shadow-lg overflow-hidden">
        ...
        <div class="bg-gradient-to-r from-blue-500 to-purple-600 p-6 text-white">
            <div class="flex justify-between items-start">
                <div class="flex-1">
                    <h1 class="text-2xl font-bold">{{ $jadwal->mataKuliah->nama_materi }}</h1>
                    <p class="text-blue-100 text-lg">Sesi {{ $jadwal->sesi_ke }} - {{ $jadwal->materi }}</p>
                    <div class="flex items-center mt-2 space-x-4">
                        <div class="flex items-center">
                            <i class="fas fa-clock mr-2"></i>
                            <span>{{ $jadwal->waktu_mulai->format('H:i') }} - {{ $jadwal->waktu_akhir->format('H:i') }}</span>
                        </div>
                        <div class="flex items-center">
                            <i class="fas fa-calendar mr-2"></i>
                            <span>{{ $jadwal->waktu_mulai->format('d F Y') }}</span>
                        </div>
                    </div>
                </div>
        ...
        <div class="p-6">
                    ...
                    <div class="bg-gray-50 rounded-lg p-4">
                        <div class="space-y-3">
                            <div class="flex justify-between border-b pb-2">
                                <span class="text-gray-600 font-medium">Mata Kuliah:</span>
                                <span class="font-semibold">{{ $jadwal->mataKuliah->nama_materi }}</span>
                            </div>
                            <div class="flex justify-between border-b pb-2">
                                <span class="text-gray-600 font-medium">Kode:</span>
                                <span class="font-mono">{{ $jadwal->mataKuliah->kode_materi }}</span>
                            </div>
                            <div class="flex justify-between border-b pb-2">
                                <span class="text-gray-600 font-medium">Sesi:</span>
                                <span class="font-semibold">Sesi {{ $jadwal->sesi_ke }}</span>
                            </div>
                            <div class="flex justify-between border-b pb-2">
                                <span class="text-gray-600 font-medium">Materi:</span>
                                <span class="text-right">{{ $jadwal->materi }}</span>
                            </div>
                            <div class="flex justify-between border-b pb-2">
                            ...
                            <div class="flex justify-between border-b pb-2">
                                ...
                                <span>{{ $jadwal->petugas->username }}</span>
                            </div>
                            <div class="flex justify-between">
                                <span class="text-gray-600 font-medium">Dibuat Pada:</span>
                                <span>{{ $jadwal->created_at->format('d/m/Y H:i') }}</span>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div>
                    ...
                    <div class="space-y-4">
                        <div class="bg-blue-50 border border-blue-200 rounded-lg p-4 text-center">
                            <div class="text-blue-600 font-bold text-2xl">{{ $jadwal->absensi->count() }}</div>
                            <div class="text-blue-800 font-medium">Total Absensi</div>
                        </div>
                        <div class="bg-green-50 border border-green-200 rounded-lg p-4 text-center">
                            <div class="text-green-600 font-bold text-2xl">{{ $jadwal->absensi->where('status', 'hadir')->count() }}</div>
                            <div class="text-green-800 font-medium">Hadir Tepat Waktu</div>
                        </div>
                        <div class="bg-yellow-50 border border-yellow-200 rounded-lg p-4 text-center">
                            <div class="text-yellow-600 font-bold text-2xl">{{ $jadwal->absensi->where('status', 'terlambat')->count() }}</div>
            ...
            <div class="mt-8">
                <h3 class="text-lg font-semibold text-gray-800 mb-4">Daftar Absensi</h3>
                        ...
                        <table class="min-w-full divide-y divide-gray-200">
                            <thead class="bg-gray-50">
                                <tr>
                                    ...
                                    <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">Waktu Absen</th>
                                    <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">Status</th>
                                </tr>
                            </thead>
                            <tbody class="divide-y divide-gray-200">
                                @foreach($jadwal->absensi as $absen)
                                <tr class="hover:bg-gray-50">
                                    ...
                                    <td class="px-4 py-3 text-sm text-gray-600">{{ $absen->peserta->kelompok ?? '-' }}</td>
